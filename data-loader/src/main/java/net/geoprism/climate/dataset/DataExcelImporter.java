...
public class DataExcelImporter
{
  ...
  public DataExcelImporter(ApplicationResource resource, ExpectedType expectedType, ExpectedHierarchy expectedHierarchy, Map<String, ShapefileFunction> columnMappings, Location[] parents, Date startDate, Date endDate, boolean ignoreFeedbackErrors, DataSource source)
  {
  ...
  public ImportHistory doImport() throws Throwable
  {
    ...
    ExcelBusinessService service = ServiceFactory.getBean(ExcelBusinessService.class);

    ServerHierarchyType hierarchyType = expectedHierarchy == null ? null : ServerHierarchyType.get(expectedHierarchy.code);

    GeoObjectImportConfiguration config = getConfiguration(resource, service, expectedType.code, ImportStrategy.UPDATE_ONLY, startDate, endDate);
  ...
  protected GeoObjectImportConfiguration getConfiguration(ApplicationResource resource, ExcelBusinessService service, String gotCode, ImportStrategy strategy, Date startDate, Date endDate)
  {
